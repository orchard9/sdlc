...
1. `POST /api/orchestrator/webhooks/routes` — write route to DB
2. `GET /api/orchestrator/webhooks/routes` — read routes from DB
3. `POST /webhooks/{route}` (existing) — store raw payload bytes in DB
