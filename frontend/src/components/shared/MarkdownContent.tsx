...
import { useProjectSettings } from '@/components/shared/ProjectSettingsContext'

// Matches relative file paths: e.g. src/foo/bar.tsx, crates/lib/src/main.rs
...
export function MarkdownContent({ content, className, showToc = false }: MarkdownContentProps) {
  const [raw, setRaw] = useState(false)
  const { projectRoot, ideUriScheme } = useProjectSettings()

  const headings = useMemo(() => (showToc ? extractHeadings(content) : []), [content, showToc])

  const makeHeadingComponent = (tag: 'h1' | 'h2' | 'h3', cls: string) =>
    ({ children }: { children?: React.ReactNode }) => {
      const text = typeof children === 'string' ? children : ''
      const slug = slugify(text || String(children ?? ''))
      const Tag = tag
      return <Tag id={slug} className={cls}>{children}</Tag>
    }

  return (
    <div className={cn('relative', showToc && headings.length > 0 ? 'flex gap-6' : '', className)}>
      {/* TOC rail — desktop sticky sidebar */}
      {showToc && headings.length > 0 && (
        <nav
          aria-label="Contents"
          className="hidden lg:block w-48 shrink-0 sticky top-0 self-start max-h-screen overflow-y-auto"
        >
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Contents</p>
          <ul className="space-y-1">
            {headings.map((h) => (
              <li key={h.slug} style={{ paddingLeft: `${(h.level - 1) * 12}px` }}>
                <a
                  href={`#${h.slug}`}
      ...
      {showToc && headings.length > 0 && (
        <div className="lg:hidden mb-3">
          <select
            ...
            className="w-full text-xs bg-muted border border-border rounded px-2 py-1 text-muted-foreground"
          >
        ...
        {raw ? (
          ...
          <div className="pt-6 text-sm text-foreground">
            <ReactMarkdown
              ...
              components={{
                h1: makeHeadingComponent('h1', 'text-xl font-bold text-foreground mt-4 mb-2 first:mt-0'),
                h2: makeHeadingComponent('h2', 'text-lg font-semibold text-foreground mt-4 mb-2 first:mt-0'),
                h3: makeHeadingComponent('h3', 'text-base font-semibold text-foreground mt-3 mb-1.5 first:mt-0'),
                ...
                code: ({ children, className: codeClass, node }) => {
                  ...
                  // F2: inline code — check for file path pattern
                  const text = String(children).trim()
                  if (FILE_PATH_PATTERN.test(text)) {
                    const href = projectRoot
                      ? `${ideUriScheme}://file/${projectRoot}/${text}`
                      : `${ideUriScheme}://file/${text}`
                    return (
                      <a
                        href={href}
                        className="text-xs font-mono bg-muted/60 border border-border/50 px-1 py-0.5 rounded text-primary underline underline-offset-2 hover:opacity-80 transition-opacity"
                      >
                        {text}
                      </a>
                    )
