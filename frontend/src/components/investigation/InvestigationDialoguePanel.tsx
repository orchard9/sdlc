  ...
  return (
    <div className="my-4 border border-primary/20 rounded-lg overflow-hidden text-xs">
      <div className="flex items-center justify-between px-3 py-1.5 bg-primary/5 border-b border-primary/20">
        <span className="font-mono font-semibold text-primary/80">sdlc_investigation_chat</span>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-end gap-2 px-4 py-3 border-t border-border bg-card">
      <textarea
        ...
        placeholder="Add context or answer questions..."
        rows={1}
...
// ---------------------------------------------------------------------------
// Main InvestigationDialoguePanel
// ---------------------------------------------------------------------------
...
export function InvestigationDialoguePanel({ entry, onRefresh }: Props) {
  ...
  const [runState, setRunState] = useState<InvestigationRunState>({ status: 'idle' })
  ...
  const loadSessions = useCallback(async () => {
    try {
      const metas: SessionMeta[] = await api.getInvestigationSessions(slug)
      const contents = await Promise.all(
        metas.map(m => api.getInvestigationSession(slug, m.session))
      )
  ...
  useSSE(handleUpdate, undefined, undefined, handleInvestigationEvent)
  ...
  const handleSend = useCallback(
    async (message: string) => {
      try {
        const res = await api.startInvestigationChat(slug, message || undefined)
  ...
  const handleStop = useCallback(async () => {
    await api.stopInvestigationChat(slug).catch(() => {})
    setRunState({ status: 'idle' })
    setPendingMessage(null)
  }, [slug])
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="shrink-0 px-5 py-3 border-b border-border/50">
        <PhaseStrip phases={phases} current={entry.phase} />
      </div>
