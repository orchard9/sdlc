...
5. Implement `--run` mode: read JSON stdin → do work → write ToolResult stdout.
   - All errors: `{ ok: false, error: "..." }` — never throw.
   - All logging via `makeLogger` to stderr only — no `console.log`.
6. Add `--setup` mode only if `requires_setup: true`; skip otherwise.
7. Write `README.md` with Usage (real commands) and Setup sections.
8. Self-check: `--meta` exits 0, all properties have descriptions, no console.log, no throw.
9. Test: `bun run .sdlc/tools/<name>/tool.ts --meta | jq .` then `echo '{}' | ... --run | jq .ok`
10. Run `/sdlc-tool-audit <name>` then `/sdlc-tool-uat <name>`.
11. `sdlc tool sync` and commit.
...
5. Implement `--run`: stdin JSON → work → ToolResult stdout. All errors: `{ ok: false, error }`. All logs: `makeLogger` (stderr, never `console.log`).
6. Add `--setup` only if `requires_setup: true`.
7. Write `README.md` with Usage (real commands) + Setup sections.
8. Self-check before audit: `--meta` exits 0, all properties have descriptions, no thrown errors.
9. Test `--meta | jq .` then `echo '{}' | ... --run | jq .ok`.
10. Audit: `/sdlc-tool-audit <name>` then UAT: `/sdlc-tool-uat <name>`.
11. `sdlc tool sync` and commit.
"#;
