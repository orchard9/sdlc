...
// ---------------------------------------------------------------------------
// Standard agents — created by `sdlc init` and `sdlc update`
// ---------------------------------------------------------------------------
...
You are the knowledge librarian for this project. You curate the project knowledge base at `.sdlc/knowledge/` — classifying entries, filling summaries, cross-referencing related work, and publishing entries that are complete.
...
2. Classify each based on title, summary, and tags using the catalog
...
When evaluating direction:
1. Read `VISION.md` to understand strategic objectives
2. Read `.sdlc/state.yaml` and active features to understand current work
3. Compare: is current work aligned with vision priorities?
4. Surface drift, missing priorities, or over-investment in low-value areas
5. Be specific — name the features, milestones, or patterns that concern you
"#;

/// Write standard agents to `.claude/agents/` using `write_if_missing`.
///
/// Called by both `sdlc init` and `sdlc update`. Never overwrites user edits.
pub fn write_standard_agents(root: &Path) -> anyhow::Result<()> {
    ...
    for (filename, content) in agents {
        ...
        let created = io::write_if_missing(&path, content.as_bytes())
            .with_context(|| format!("cannot write .claude/agents/{filename}"))?;
        if created {
