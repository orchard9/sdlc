...
use sdlc_core::telegram::{
    get_me, poll_loop, DigestConfig, DigestRunner, MessageStore, SmtpConfig, TelegramConfig,
};
use std::path::Path;
...
pub enum TelegramSubcommand {
    ...
    /// Fetch messages from configured Telegram chats and send a digest email via SMTP.
    ///
    /// Requires: TELEGRAM_BOT_TOKEN (or telegram.bot_token in config) and SMTP credentials.
...
fn run_digest(
    ...
    let digest_config = build_digest_config(config, window_override, chat_override)?;
...
fn build_digest_config(
    ...
    let smtp_yaml = tg.and_then(|t| t.smtp.as_ref());

    let bot_token = std::env::var("TELEGRAM_BOT_TOKEN")
        .ok()
        .or_else(|| tg.and_then(|t| t.bot_token.clone()))
        .context(
            "Telegram bot token not configured. Set TELEGRAM_BOT_TOKEN env var or add \
             telegram.bot_token to .sdlc/config.yaml",
        )?;

    let smtp_host = std::env::var("SMTP_HOST")
        .ok()
        .or_else(|| smtp_yaml.and_then(|s| s.host.clone()))
        .context(
            "SMTP host not configured. Set SMTP_HOST env var or add \
             telegram.smtp.host to .sdlc/config.yaml",
        )?;

    let smtp_port: u16 = std::env::var("SMTP_PORT")
        .ok()
        .and_then(|v| v.parse().ok())
        .or_else(|| smtp_yaml.and_then(|s| s.port))
        .unwrap_or(587);

    let smtp_username = std::env::var("SMTP_USERNAME")
        .ok()
        .or_else(|| smtp_yaml.and_then(|s| s.username.clone()))
        .context(
            "SMTP username not configured. Set SMTP_USERNAME env var or add \
             telegram.smtp.username to .sdlc/config.yaml",
        )?;

    let smtp_password = std::env::var("SMTP_PASSWORD")
        .ok()
        .or_else(|| smtp_yaml.and_then(|s| s.password.clone()))
        .context(
            "SMTP password not configured. Set SMTP_PASSWORD env var or add \
             telegram.smtp.password to .sdlc/config.yaml",
        )?;

    let smtp_from = std::env::var("SMTP_FROM")
        .ok()
        .or_else(|| smtp_yaml.and_then(|s| s.from.clone()))
        .context(
            "SMTP from address not configured. Set SMTP_FROM env var or add \
             telegram.smtp.from to .sdlc/config.yaml",
        )?;

    let smtp_to: Vec<String> = std::env::var("SMTP_TO")
        .ok()
        .map(|v| v.split(',').map(|s| s.trim().to_string()).collect())
        .or_else(|| smtp_yaml.map(|s| s.to.clone()).filter(|v| !v.is_empty()))
        .context(
            "SMTP recipients not configured. Set SMTP_TO env var (comma-separated) or add \
             telegram.smtp.to to .sdlc/config.yaml",
        )?;
